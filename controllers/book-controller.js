...
const getAllBooks = async(req,res)=>{
    try{
        ...
        if(allBooks.length>0)
        {
            ...
            res.status(404).json({
                success : false,
                message : 'Bo book found in collection'
            });
        }
    }catch(e)
    {
...
const addNewBooks = async(req,res)=>{
    try{
        ...
        const newlyCretaedBook = await Book.create(newBookFormData);
        if(newlyCretaedBook)
        {
            res.status(201).json({
                success : true,
                message : 'Book added successfully',
                data : newlyCretaedBook,
            });
        }
    }catch(e)
    {
...
module.exports = {getAllBooks, getSingleBooks, addNewBooks, updateBooks,deleteBooks};